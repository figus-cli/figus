import React, { SVGProps } from "react";

export function SignOutSqureDuotone(props: SVGProps<SVGSVGElement>) {
    return (
        <svg
            fill="currentColor"
            viewBox="0 0 24 24"
            width="1em"
            height="1em"
            {...props}
        >
            <path
                fill="currentColor"
                d="m10.3675 6.5442 6.3163-2.1055C17.3313 4.223 18 4.705 18 5.3874v13.2252c0 .6825-.6687 1.1645-1.3162.9487l-6.3163-2.1055A2 2 0 0 1 9 15.5585v-7.117a2 2 0 0 1 1.3675-1.8973Z"
                fillOpacity=".24"
            />
            <path
                fill="currentColor"
                d="M6.8536 9.8536a.5.5 0 1 0-.7072-.7072l.7072.7072ZM4 12l-.3535-.3536L3.2929 12l.3536.3536L4 12Zm2.1464 2.8536a.5001.5001 0 0 0 .7072-.7072l-.7072.7072ZM14 12.5a.5.5 0 0 0 0-1v1ZM6.1464 9.1464l-2.5 2.5.7072.7072 2.5-2.5-.7072-.7072Zm-2.5 3.2072 2.5 2.5.7072-.7072-2.5-2.5-.7071.7072ZM4 12.5h10v-1H4v1Z"
            />
        </svg>
    );
}
export default SignOutSqureDuotone;
